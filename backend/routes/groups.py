...
import asyncio
from db import Exploit, AttackExecution
...
async def group_get(db: DBSession):
    ...
    async def result(result: sqla.Row[Tuple[Exploit, AttackExecution]]):
        ...
        if memebers is None or len(memebers) == 0:
            ...
            memebers = set()
        commit_id = await redis_conn.get(f"group:{group.id}:commit")
...
async def new_group(data: AddGroupForm, db: DBSession):
    ...
    await redis_conn.set(f"group:{group.id}", True)
    await db.commit()
...
async def delete_group(group_id: AttackGroupID, db: DBSession):
    ...
    await redis_conn.delete(*(await redis_conn.keys(f"group:{result.id}:*")))
    await db.commit()
